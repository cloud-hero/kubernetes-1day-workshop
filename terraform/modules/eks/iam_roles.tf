...
resource "aws_iam_role" "cluster_autoscaler" {
  name = "${var.project_name}-${var.cluster_name}-Cluster-Autoscaler"
  ...
  assume_role_policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Condition": {
        "StringEquals": {
          "${replace(aws_eks_cluster.this.identity.0.oidc.0.issuer, "https://", "")}:sub": "system:serviceaccount:${var.cluster_autoscaler_sa}",
